...
import quasar.fs.mount.{MountConfig, MountType, ConnectionUri}
...
package object metastore {
  ...
  def verifyMetaStoreSchema[A](schema: Schema[A]): EitherT[ConnectionIO, String, Unit] =
    EitherT(schema.updateRequired map {
      case Some((None, _)) =>
        "MetaStore requires initialization, try running the 'initUpdateMetaStoreSchema' command.".left
  ...
  implicit val mountConfigComposite: Composite[MountConfig] =
    Composite[(String, String)].xmap(
      { case (typ, uri) =>
        MountConfig.fromConfigPair(typ, ConnectionUri(uri))
